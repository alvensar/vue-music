    ...
    <div class="p-6">
      ...
      <div
        class="w-full px-10 py-20 rounded text-center cursor-pointer border border-dashed
          ...
          hover:bg-green-400 Shover:border-green-400 hover:border-solid"
  ...
  methods: {
    upload($event) {
      ...
      files.forEach((file) => {
        ...
        const storageRef = storage.ref(); // music-app-1d0de.appspot.com
        const songsRef = storageRef.child(`songs/${file.name}`); // music-app-1d0de.appspot.com/songs/example.mp3
        songsRef.put(file);
      });
      console.log(files);
    },
